...
contract DealClient {
    ...
    function authenticateMessage(bytes memory params) internal view {
        ...
        (uint256 proposalStoragePricePerEpoch, bool storagePriceConverted) =
            BigInts.toUint256(proposal.storage_price_per_epoch);
        (uint256 proposalClientCollateral, bool collateralConverted) =
            BigInts.toUint256(proposal.storage_price_per_epoch);
        require(
        ...
        require(
            proposalStoragePricePerEpoch <= req.storage_price_per_epoch, "storage price greater than request amount"
        );
    ...
    function withdrawBalance(address client, uint256 value) public returns (uint256) {
        require(msg.sender == owner);

        MarketTypes.WithdrawBalanceParams memory params =
            MarketTypes.WithdrawBalanceParams(FilAddresses.fromEthAddress(client), BigInts.fromUint256(value));
